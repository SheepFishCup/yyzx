...
if exist "%AGENT_JAR%" (set AGENT_OPTS=-javaagent:%AGENT_JAR% -Dskywalking.agent.service_name=yyzx-checkinout -Dskywalking.collector.backend_service=localhost:11800) else (set AGENT_OPTS=)
cd /d "%~dp0..\yyzx-checkinout"
echo [INFO] Starting yyzx-checkinout
