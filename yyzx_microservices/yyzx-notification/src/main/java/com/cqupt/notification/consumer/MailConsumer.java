...
import com.cqupt.notification.websocket.WebSocketServer;
...
public class MailConsumer {
    ...
    private void sendPasswordResetNotify(String email, String username) {
        try {
            // 根据邮箱查询用户（通过 Feign 调用 yyzx-auth）
            ResultVo<User> result = userFeignClient.getByEmail(email);
            User user = result != null ? result.getData() : null;
